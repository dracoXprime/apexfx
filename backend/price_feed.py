"""APEX FX - Price Feed (MT5 primary · Twelve Data fallback · Simulation)"""
...
# MT5 symbol names → internal pair names
MT5_SYMBOL_MAP = {
    "EURUSD":"EURUSD","GBPUSD":"GBPUSD","USDJPY":"USDJPY",
    "AUDUSD":"AUDUSD","USDCAD":"USDCAD","GBPJPY":"GBPJPY",
    "XAUUSD":"XAUUSD","GOLD":"XAUUSD",
}
MT5_TF_MAP = {"M15":"M15","H1":"H1","H4":"H4","PERIOD_M15":"M15","PERIOD_H1":"H1","PERIOD_H4":"H4"}
...
class PriceFeed:
    def __init__(self):
        ...
        self._sim_candles: dict = {}   # simulation / twelve data candles
        self._mt5_candles: dict = {}   # MT5-provided candles {pair:{tf:[candles]}}
        self._mt5_prices:  dict = {}   # MT5-provided prices  {pair:{...}}
        self._mt5_last_update: Optional[datetime] = None

    # ── MT5 data ingestion ────────────────────────────────────────────────────

    def ingest_mt5(self, payload: dict):
        """
        Called by the /api/mt5/push endpoint whenever MT5 EA sends data.
        payload = {
            "pair": "EURUSD",
            "tf": "H1",
            "candles": [...],   # list of {time, open, high, low, close, volume}
            "price": {...}       # optional current price {bid, ask, mid}
        }
        """
        pair = MT5_SYMBOL_MAP.get(payload.get("pair","").upper())
        tf   = MT5_TF_MAP.get(payload.get("tf","").upper())
        if not pair or not tf:
            log.warning(f"MT5 push: unknown pair/tf {payload.get('pair')}/{payload.get('tf')}")
            return False
        ...
        if candles:
            ...
            log.info(f"MT5 data received: {pair}/{tf} — {len(candles)} candles")

        price = payload.get("price")
        if price and pair in PAIRS:
            info = PAIRS[pair]
            mid = float(price.get("mid", price.get("ask",0)))
            if mid > 0:
                self._mt5_prices[pair] = self._mk_price(pair, mid, info)

        return True

    def ingest_mt5_batch(self, payloads: list):
        """Accept a list of pair/tf payloads in one call."""
        for p in payloads:
            self.ingest_mt5(p)

    def mt5_connected(self) -> bool:
        """True if MT5 data was received in the last 5 minutes."""
        if not self._mt5_last_update: return False
    ...
    def _gen_history(self, pair, info, tf_mins, count):
        price = info["base"]; vol = info["pip"]*8
        candles = []; now = datetime.now(timezone.utc)
    ...
    async def init(self):
        ...
        if self.key:
            ...
            log.info("Twelve Data API key found — fetching live candles")
            await self._fetch_all_candles()
        else:
            log.info("No API key — simulation mode")

    async def tick(self):
        """Update prices every second. MT5 prices take priority."""
        if self.mt5_connected():
            self.source = "mt5"
            # MT5 prices already updated via ingest — just tick sim for fallback
    ...
    def get_prices(self) -> dict:
        """MT5 prices take priority over Twelve Data / simulation."""
        base = dict(self._sim_prices)
        base.update(self._mt5_prices)
        return base

    def get_candles(self, pair, tf) -> list:
        """MT5 candles take priority."""
        mt5 = self._mt5_candles.get(pair, {}).get(tf)
        if mt5: return list(mt5)
        return list(self._sim_candles.get(pair, {}).get(tf, []))

    def get_all_candles(self) -> dict:
        return {p: {tf: self.get_candles(p, tf) for tf in TIMEFRAMES} for p in PAIRS}

    def get_price(self, pair) -> dict:
